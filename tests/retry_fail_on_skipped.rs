...
async fn fails() {
    ...
    // A `Step` matching no function is skipped, and skipping aborts its
    // `Scenario`, so only the first `Step` is reported (as failed, because of
    // `fail_on_skipped()`), and a skipped `Scenario` is not retried.
