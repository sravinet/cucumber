...
pub mod feature_support {
    ...
    pub fn enabled_features() -> Vec<&'static str> {
        ...
        if has_json_output() {
            features.push("output-json");
        }
        if has_junit_output() {
            features.push("output-junit");
        }
        if has_libtest() {
            features.push("libtest");
        }
        if has_timestamps() {
...
mod output_features_tests {
    ...
    fn test_output_format_availability() {
        ...
        #[cfg(feature = "output-json")]
        {
            let _json_writer = cucumber::writer::Json::<io::Sink>::new::<
                OutputTestWorld,
            >(io::sink());
        ...
        #[cfg(feature = "output-junit")]
        {
            let _junit_writer = cucumber::writer::JUnit::<
                OutputTestWorld,
                io::Sink,
            >::new(io::sink(), 1);
        ...
        #[cfg(feature = "libtest")]
        {
            let _libtest_writer = cucumber::writer::Libtest::<
                OutputTestWorld,
                io::Sink,
            >::new(io::sink());
    ...
    fn test_junit_with_timestamps() {
        ...
        let _junit_writer =
            cucumber::writer::JUnit::<OutputTestWorld, io::Sink>::new(
                io::sink(),
                1,
            );
...
mod observability_tests {
    ...
    fn test_observability_features_available() {
        ...
        assert!(
            combo.has_observability_features(),
...
mod summary_tests {
    ...
    fn test_feature_combination_display() {
        ...
        assert_eq!(
            combos[2].to_string(),
            "output-json,output-junit,timestamps"
        );
    }
    ...
    fn test_current_feature_summary() {
        ...
        if combo.output_json {
            println!("  ✓ JSON output enabled");
        }
        if combo.output_junit {
            println!("  ✓ JUnit output enabled");
        }
        if combo.libtest {
            println!("  ✓ Libtest output enabled");
        }
        if combo.timestamps {
        ...
        if combo.observability {
            println!("  ✓ Observability enabled");
        }
        if combo.macros {
            println!("  ✓ Macros enabled");
        }
