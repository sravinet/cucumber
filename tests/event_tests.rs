...
fn test_event_new_creates_event_correctly() {
    ...
    #[cfg(feature = "timestamps")]
    {
        ...
        let diff = now
            ...
            .unwrap_or_else(|_| event.at.duration_since(now).unwrap());
...
fn test_event_split() {
    ...
    #[cfg(feature = "timestamps")]
    {
        let now = std::time::SystemTime::now();
        let diff = now
            .duration_since(metadata.at)
            .unwrap_or_else(|_| metadata.at.duration_since(now).unwrap());
...
fn test_hook_events() {
    ...
    let failed =
        Hook::<TestWorld>::Failed(Some(Arc::new(TestWorld)), Arc::new("error"));
...
fn test_scenario_events() {
    ...
    let hook_event =
        Scenario::<TestWorld>::Hook(HookType::Before, Hook::Started);
...
fn test_step_error_coercion() {
    ...
    let errors = vec![
        StepError::NotFound,
        StepError::AmbiguousMatch(step::AmbiguousMatchError {
            possible_matches: vec![],
        }),
