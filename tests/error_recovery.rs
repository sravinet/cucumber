...
use cucumber::{
    World,
    error::{
        CucumberError, ExecutionError, ExecutionResult, WriterError,
        WriterResult,
    },
};
...
mod writer_error_recovery {
    ...
    fn test_unavailable_error_recovery() {
        ...
        assert_eq!(
            writer_error.unavailable_reason(),
            Some("output buffer full")
        );
...
mod integration_error_recovery {
    ...
    impl ErrorProneSystem {
        ...
        fn attempt_operation(&mut self) -> Result<String, CucumberError> {
            ...
            if self.should_fail_io {
                ...
                return Err(CucumberError::Writer(WriterError::Io(
                    io::Error::new(io::ErrorKind::BrokenPipe, "pipe failed"),
                )));
            }
    ...
    fn test_multiple_error_recovery() {
        let mut system =
            ErrorProneSystem::new().with_execution_failure().with_io_failure();
...
mod error_recovery_performance {
    ...
    fn test_error_creation_performance() {
        ...
        for i in 0..1000 {
            let _error =
                ExecutionError::feature_not_found(&format!("feature_{}", i));
        }
    ...
    fn test_result_unwrap_or_else_performance() {
        ...
        for i in 0..1000 {
            let result: Result<String, ExecutionError> = if i % 2 == 0 {
                ...
                Err(ExecutionError::feature_not_found(&format!(
                    "feature_{}",
                    i
                )))
            };
...
mod comprehensive_error_tests {
    ...
    fn test_error_recovery_world_integration() {
        ...
        let errors = vec![
            CucumberError::Execution(ExecutionError::feature_not_found(
                "test1",
            )),
            CucumberError::Writer(WriterError::unavailable("buffer full")),
            CucumberError::Execution(ExecutionError::state_inconsistency(
                "a", "b",
            )),
        ];
        ...
        for (i, error) in errors.into_iter().enumerate() {
            match error {
                CucumberError::Execution(e) => {
                    world
                        .record_error(&format!("execution_error_{}: {}", i, e));
                }
                ...
                _ => {
                    world
                        .record_error(&format!("other_error_{}: {}", i, error));
                }
            }
        }
