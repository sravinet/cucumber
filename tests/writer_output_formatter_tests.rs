...
impl FailingWriter {
    fn new(fail_mode: FailMode) -> Self {
        Self { buffer: Vec::new(), fail_mode, call_count: 0 }
    }
...
fn output_formatter_write_fmt_edge_cases() {
    ...
    // Complex formatting
    assert!(
        OutputFormatter::write_fmt(
            &mut writer,
            format_args!("{:?} {:x} {:.2}", vec![1, 2, 3], 255, 3.14159)
        )
        .is_ok()
    );
...
fn writer_ext_error_handling_with_various_error_types() {
    ...
    let parse_result: Result<(), std::num::ParseIntError> =
        "not_a_number".parse::<i32>().map(|_| ());
