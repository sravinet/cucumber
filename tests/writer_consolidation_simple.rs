...
fn shared_utilities_are_accessible() {
    use cucumber::writer::{CommonWriterExt, ErrorFormatter, WorldFormatter};

    // Test WorldFormatter
    let world = Some(&42);
    let result = WorldFormatter::format_world_if_needed(
        world,
        cucumber::writer::Verbosity::ShowWorld,
    );
...
fn world_formatter_handles_different_types() {
    use cucumber::writer::{Verbosity, WorldFormatter};
    ...
    assert!(
        WorldFormatter::format_world_if_needed(int_world, Verbosity::ShowWorld)
            .is_some()
    );
    assert!(
        WorldFormatter::format_world_if_needed(
            string_world,
            Verbosity::ShowWorld
        )
        .is_some()
    ...
    assert!(
        WorldFormatter::format_world_if_needed(int_world, Verbosity::Default)
            .is_none()
    );
    assert!(
        WorldFormatter::format_world_if_needed(
