...
fn validate_junit_structure(xml_content: &str) {
    ...
    assert_eq!(
        ...
        "All test cases should be accounted for"
    );
...
fn validate_junit_essential_content(xml_content: &str) {
    ...
    assert!(
        xml_content.contains("testsuites"),
        "Must contain testsuites root element"
    );
    assert!(
        xml_content.contains("testsuite"),
        "Must contain testsuite elements"
    );
    assert!(xml_content.contains("testcase"), "Must contain testcase elements");

    // Validate feature-related content
    assert!(
        xml_content.contains("Feature:"),
        "Must contain feature information"
    );
    assert!(
        xml_content.contains("Scenario:"),
        "Must contain scenario information"
    );

    // Validate expected error types
    // Note: Parser errors are only present if there are actual parsing errors in feature files
    assert!(
        xml_content.contains("Step Panicked"),
        "Must contain step panic information"
    );

    // Validate failure content
    assert!(
        xml_content.contains("Too much!"),
        "Must contain expected step failure message"
    );
    assert!(
        xml_content.contains("doesn't match any function"),
        "Must contain step matching failure"
    );

    // Validate structural elements
    assert!(xml_content.contains("failure"), "Must contain failure elements");
    assert!(
        xml_content.contains("system-out"),
        "Must contain system-out for successful tests"
    );
    ...
    assert!(
        xml_content.contains("tests="),
        "Must contain test count attributes"
    );
    assert!(
        xml_content.contains("failures="),
        "Must contain failure count attributes"
    );

    println!(
        "JUnit semantic validation passed - all essential content present"
    );
}
