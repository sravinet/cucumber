...
impl<'a, W> StepContext<'a, W> {
    ...
    pub fn with_captures(
        mut self,
        captures: Option<&'a CaptureLocations>,
    ) -> Self {
...
mod tests {
    ...
    fn step_context_scenario_type_detection() {
        ...
        let scenario_type_no_examples = if empty_examples_count == 0 {
            "scenario"
        } else {
            "scenario outline"
        };
        let scenario_type_with_examples = if non_empty_examples_count == 0 {
            "scenario"
        } else {
            "scenario outline"
        };
    ...
    fn scenario_context_scenario_type_detection() {
        ...
        let scenario_type =
            if has_examples { "scenario outline" } else { "scenario" };
