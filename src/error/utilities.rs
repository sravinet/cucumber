...
pub mod conversion {
    ...
    pub fn any_error_with_context<E>(error: E, context: &str) -> CucumberError
    where
        E: std::error::Error + Send + Sync + 'static,
    {
        CucumberError::Config(ConfigError::InvalidCliArgs {
...
mod tests {
    ...
    fn test_result_config_ext() {
        let ok_result: std::result::Result<String, TestError> =
    ...
    fn test_conversion_io_error_with_context() {
        ...
        let cucumber_err =
            conversion::io_error_with_context(io_err, "reading config file");
    ...
    fn test_conversion_any_error_with_context() {
        ...
        let cucumber_err =
            conversion::any_error_with_context(test_err, "processing data");
    ...
    fn test_formatting_truncate_error_message() {
        ...
        assert_eq!(
            formatting::truncate_error_message(short_message, 10),
            "short"
        );
    ...
    fn test_multiple_extensions_usage() {
        let result: std::result::Result<i32, TestError> =
            Err(TestError("test".to_string()));

        // Test chaining different context methods
        let cucumber_result = result.with_cucumber_context("cucumber context");
        assert!(cucumber_result.is_err());

        let result2: std::result::Result<i32, TestError> =
            Err(TestError("test2".to_string()));
